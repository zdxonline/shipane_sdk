...
from shipane_sdk.jobs.update_info_job import UpdateInfoJob
from shipane_sdk.jobs.grid_job import GridJob
...
class Scheduler(object):
    ...
    def __create_job(self, section):
        ...
        elif job_type == 'UpdateInfoJob':
            job = self.__create_new_updae_info_job(section)
        elif job_type =='GridJob':
            job=self.__create_new_grid_job(section)
        return job

    def __create_new_grid_job(self, section):
        options = self.__build_options(section)
        options['db_path'] = self._db_path
        client_aliases = self.__filter_client_aliases(section)
        return GridJob(self._client, client_aliases, '{}-Job'.format(section), **options)

    def __create_new_updae_info_job(self, section):
        options = self.__build_options(section)
        options['db_path'] = self._db_path
        client_aliases = self.__filter_client_aliases(section)
        return UpdateInfoJob(self._client, client_aliases, '{}-Job'.format(section), **options)
...
def start():
    logging.config.fileConfig(os.path.join(BASE_PATH, 'conf', 'logging.ini'))
